...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private val locationRequestPermission by lazy {
        permissionsBuilder(Manifest.permission.ACCESS_COARSE_LOCATION,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(locationRequestPermission.checkStatus().allDenied()) {
            Toast.makeText(this,"tbnk :(",Toast.LENGTH_LONG)
            finish()
        }
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        val meteoMarkerOptions = MarkerOptions().position(position).icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_BLUE))
        mMap.addMarker(meteoMarkerOptions)

        val cameraOptions = CameraPosition.Builder().target(position).zoom(9f).build()
